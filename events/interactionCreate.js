...
async function closeTicket(channel, closedBy) {
  ...
  if (channel.ticketOpener) {
    closeEmbed.addFields(
      {
        name: "Opened By",
        value: `<@${channel.ticketOpener.id}>`,
        inline: true,
      },
      {
        name: "Opened At",
        value: `<t:${Math.floor(channel.openTime / 1000)}:F>`,
        inline: true,
      }
    );
  }
  ...
  const transcriptChannel = await channel.guild.channels.fetch(
    config.tickets.transcriptChannel
  );
  ...
  if (channel.ticketOpener) {
    try {
      ...
      console.error(
        "Error sending close notification to ticket opener:",
        error
      );
    }
  }
  ...
  async execute(interaction) {
    if (interaction.isChatInputCommand()) {
      ...
      if (interaction.customId === "ticket_select") {
        ...
        const ticketType = config.tickets.ticketTypes.find(
          (type) => type.name.toLowerCase().replace(/ /g, "_") === selectedValue
        );
      ...
      if (interaction.customId.startsWith("ticket_modal_")) {
        ...
        const ticketType = config.tickets.ticketTypes.find(
          (type) =>
            type.name.toLowerCase().replace(/ /g, "_") === ticketTypeValue
        );
        ...
        try {
          ...
          const hasOpenTicket = await userHasOpenTicket(
            guild.id,
            interaction.user.id
          );
          if (hasOpenTicket) {
            await interaction.reply({
              content:
                "You already have an open ticket. Please use your existing ticket or close it before creating a new one.",
              ephemeral: true,
            });
            return;
          }
      ...
      if (interaction.customId === "close_ticket") {
        ...
        try {
          ...
          await interaction.editReply({
            content: "The transcript has been sent to your DMs.",
            ephemeral: true,
          });
          ...
          const channelEmbed = new EmbedBuilder()
            .setColor(0x0099ff)
            .setDescription(
              `${interaction.user} has requested a transcript of this ticket.`
            )
            .setTimestamp();
          ...
          if (error.code === 50007) {
            ...
            await interaction.editReply({
              content:
                "There was an error creating the transcript. Please try again.",
              ephemeral: true,
            });
          }
        }
